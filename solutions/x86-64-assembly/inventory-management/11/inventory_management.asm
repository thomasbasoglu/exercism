...
get_box_weight:
    ...
    movzx edi, di    ; 1st product count
    movzx esi, si    ; 1st product weight
    movzx edx, dx    ; 2nd product count
    movzx ecx, cx    ; 2nd product weight
    
    imul edi, esi ; getting first argument times the second 
    imul edx, ecx ; getting third argument times the fourth 
    
    mov eax, WEIGHT_OF_EMPTY_BOX ; add box weight to the return value
    add eax, edi ; adding to the result 
    add eax, edx ; adding to the result 
...
max_number_of_boxes:
    ...
    mov ax, TRUCK_HEIGHT
    div dil ; Divide the cm to the max_height then we can get how many boxes we can fit
    movzx eax, al 
    ret
...
calculate_payment:
    ...
    movsxd rsi, esi      ; Boxes
    movsxd rdx, edx      ; Trips
    movsxd rcx, ecx      ; Lost Items


  
    imul rsi, PAY_PER_BOX
    imul rdx, PAY_PER_TRUCK_TRIP
    add rsi, rdx       ; total cost
    ...
    ; Final step division 
    mov rax, rsi
    movzx r9, r9b ; 8 bit to 64 with movzx Number of workers
    inc r9              ; Since I count as a worker
    
    cqo 
    idiv r9
    add rax, rdx     ; Claim the remainder
